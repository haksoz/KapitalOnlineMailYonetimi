<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Faturalanmış Siparişler API Önizleme</h2>
    </x-slot>

    <div class="py-6 px-4 sm:px-6 lg:px-8 space-y-6">

        <div class="bg-green-50 border border-green-200 rounded-lg p-4 text-sm text-green-800">
            <strong>Kapsam:</strong> Yalnızca <span class="font-mono font-semibold">invoiced</span> siparişler.
            Master sistem bu veriyi bir kez alır ve kendi kaydını oluşturur; sonraki sync'lerde tekrar sorgulamaz.
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                <div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                    ...
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">invoiced</span>
                </div>
            ...
            <div class="p-4 overflow-x-auto">
                <table class="min-w-full text-xs text-left">
                    ...
                    <tbody class="divide-y divide-gray-100 text-gray-700">
                        ...
                        <tr><td class="py-1.5 pr-4 font-mono text-slate-700">actual_sales_tl</td><td class="py-1.5">Kesinleşen satış tutarı (TL)</td></tr>
                        <tr class="bg-green-50"><td class="py-1.5 pr-4 font-mono text-slate-700">sales_invoice.order_number</td><td class="py-1.5">Sistem fatura takip numarası (FTN...)</td></tr>
                        <tr class="bg-green-50"><td class="py-1.5 pr-4 font-mono text-slate-700">sales_invoice.our_invoice_number</td><td class="py-1.5">Gerçek fatura numarası</td></tr>
                        <tr class="bg-green-50"><td class="py-1.5 pr-4 font-mono text-slate-700">sales_invoice.our_invoice_date</td><td class="py-1.5">Gerçek fatura tarihi (YYYY-MM-DD)</td></tr>
                        <tr class="bg-green-50"><td class="py-1.5 pr-4 font-mono text-slate-700">sales_invoice.total_amount_tl</td><td class="py-1.5">Fatura toplam tutarı (TL)</td></tr>
                        <tr class="bg-green-50"><td class="py-1.5 pr-4 font-mono text-slate-700">sales_invoice.line_amount_tl</td><td class="py-1.5">İlgili satır tutarı (TL)</td></tr>
                        <tr class="bg-green-50"><td class="py-1.5 pr-4 font-mono text-slate-700">sales_invoice.invoiced_at</td><td class="py-1.5">Fatura oluşturma tarihi (ISO 8601)</td></tr>
    ...
    <script>
        ...
        async function loadData(page = 1) {
            const url = new URL('{{ route("admin.integration.invoiced-order-preview.data") }}', window.location.origin);
        ...
        function renderList() {
            ...
            container.innerHTML = allData.map((item, i) => {
                const actual = item.actual_sales_tl != null ? Number(item.actual_sales_tl).toLocaleString('tr-TR', {minimumFractionDigits: 2}) + ' ₺' : '-';
                return `<div onclick="selectItem(${i})" class="px-4 py-3 cursor-pointer hover:bg-slate-50 transition-colors ${selectedIndex === i ? 'bg-slate-100' : ''}">
                    <div class="flex items-start justify-between gap-2">
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-xs text-gray-400 mt-0.5">Satış: ${actual} &bull; Fatura: ${item.sales_invoice?.order_number ?? '-'}</p>
                        </div>
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800 shrink-0">invoiced</span>
                    </div>
                </div>`;
            }).join('');
        }
