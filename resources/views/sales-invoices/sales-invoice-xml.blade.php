    ...
    <div class="bg-white rounded-xl shadow-sm p-6 max-w-lg">
        <form action="{{ route('sales-invoices.store-sales-invoice-xml') }}" method="POST" enctype="multipart/form-data">
            ...
            <div>
                ...
                <input type="file" id="xml_file" name="xml_file" accept=".xml,application/xml,text/xml" class="mt-1 block w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:font-semibold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200" required>
                <p class="mt-1 text-xs text-gray-500">Satış e-fatura (UBL), en fazla 10 MB.</p>
