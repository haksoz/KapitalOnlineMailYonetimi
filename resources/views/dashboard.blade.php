            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div class="border border-gray-200 rounded-lg p-4">
                    ...
                    <dl class="mt-2 space-y-1 text-sm">
                        ...
                        <div class="flex justify-between">
                            <dt class="text-gray-500">Bu ay bilançosu (Satış − Alış)</dt>
                            @php
                                $mBal = $stats['this_month_balance'] ?? 0;
                                $mBalClass = $mBal >= 0 ? 'text-emerald-700' : 'text-rose-700';
                            @endphp
                            <dd class="font-medium {{ $mBalClass }}">
                                {{ number_format($mBal, 2, ',', '.') }} ₺
                            </dd>
                        </div>
                    </dl>
                </div>
                <div class="border border-gray-200 rounded-lg p-4">
                    <div class="text-xs font-medium text-gray-500 uppercase tracking-wide">
                        Gelecek Ay — {{ $stats['next_month_label'] ?? now()->copy()->addMonthNoOverflow()->locale('tr')->translatedFormat('F Y') }}
                    </div>
                    <dl class="mt-2 space-y-1 text-sm">
                        <div class="flex justify-between">
                            <dt class="text-gray-500">Beklenen satış (KDV hariç)</dt>
                            <dd class="font-medium text-gray-900">
                                {{ number_format($stats['next_month_expected_sales'] ?? 0, 2, ',', '.') }} ₺
                            </dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500">Beklenen alış (KDV hariç)</dt>
                            <dd class="font-medium text-gray-900">
                                {{ number_format($stats['next_month_expected_purchases'] ?? 0, 2, ',', '.') }} ₺
                            </dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500">Gelecek ay bilançosu (Satış − Alış)</dt>
                            @php
                                $nBal = $stats['next_month_balance'] ?? 0;
                                $nBalClass = $nBal >= 0 ? 'text-emerald-700' : 'text-rose-700';
                            @endphp
                            <dd class="font-medium {{ $nBalClass }}">
                                {{ number_format($nBal, 2, ',', '.') }} ₺
                            </dd>
                        </div>
                    </dl>
                </div>
